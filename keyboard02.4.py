...
class MilleniumChess(ShowBase):
    ...
    #                     count+= 1
                    ...
                    if y == 12 and z != 0  and z != -1:
                        ...
                        self.records["whitePawn"] = self.pieces[(cord)]
...
#         self.colorOfPiece
    ...
    # Check whether a square is being attacked by opponent pieces
    def checkThreat(self, square):
        potentialThreats = None
        if self.colorOfPiece[:-1] == (1,1,1):
            self.piece = self.records["lightPawn"]
        elif self.colorOfPiece[:-1] != (1,1,1):
            self.piece = self.records["darkPawn"]
            
        potentialThreats1 = self.findValidBishopSq()
        if self.checkPiece(potentialThreats1, "bishop"):
            return True
        potentialThreats = self.findKnightMoves()
        if self.checkPiece(potentialThreats, "knight"):
            return True
        potentialThreats2 = self.findValidRookSq()
        if self.checkPiece(potentialThreats2, "rook"):
            return True
        potentialThreats = potentialThreats1 + potentialThreats2
        if self.checkPiece(potentialThreats, "queen"):
            return True
#         potentialThreats = self.findValidBishopSq()
#         if self.checkPiece(potentialThreats, "bishop"):
#             return True
...
# Removes the tuple cords that are outside the boards and tuple cords where the alies reside
    def removeExtra(self, moves):
        for move in moves[:]:
            if move not in self.squares:
                moves.remove(move)
            elif move in self.pieces and self.pieces[move].getColor() == self.colorOfPiece:
                moves.remove(move)
        return moves
    ...
    def showValidSq(self, validMoves):
        for move in validMoves:
            if isinstance(move, tuple()) == False: 
                move.setColor(0, 1, 0)
    ...
    def findValidBishopSq(self):# Changes the color of valid squares of bishop into green
