...
# Moving a piece in all diection without subsituting error
...
#         print(self.sq)
        ...
        self.accept("enter-up", updateKeyMap, ["enter", False])
    ...
    def update(self, task):
        ...
        if keyMap["enter"]:
            for i in range(64):
#                 print(self.pieces[index])
                if isinstance(self.pieces[i], int) == False:
                    posOfPiece = self.pieces[i].getPos()
                    if self.compare(posOfPiece, posOfSq)  and self.select == None :
                        self.select = i
                        break
#             
