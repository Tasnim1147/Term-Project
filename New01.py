        ...
        if self.turn == 1:
            if self.select == False:
                if self.pieces[cord] == None:
                    ...
                    return
                elif self.pieces[cord] != None:
                    color = roundTuple(self.pieces[cord].getColor())
                    if color != (1, 1, 1):
                        keyMap["enter"] = False # Does not do any thing if an empty sq is selected
                        return
                    else:
    ...
    #             if cord not in self.pieces:
            ...
            if self.select == False:
                if self.inverseP[cord] == None:
                    ...
                    keyMap["enter"] = False # Does not do any thing if an empty sq is selected
                    return
                elif self.inverseP[cord] != None:
#                     print("not none?")
                    color = roundTuple(self.inverseP[cord].getColor())
                    if color == (1, 1, 1):
                        keyMap["enter"] = False # Does not do any thing if an empty sq is selected
                        return
                    else:
    ...
    #             if cord not in self.pieces:
                if roundTuple(self.inverseS[cord].getColor()) == (0, 1, 0):
                    if not self.moveCondition(cord):
                        if self.inverseP[cord] != None:
                            self.pCord[self.inverseP[cord]] = (100, 100, 100)
                            self.inverseP[cord].setPos(100, 100, 100)
                        self.inverseP[cord] = self.piece # Replacing/ placing the piece
                        self.pCord[self.piece] = cord
                        if cord != self.pieceKey: # If not then the piece will be lost from the dictionary
                            self.inverseP[self.pieceKey] = None # Change the previous place to None
                        self.select = False # Now we donot have the piece
                        self.piece = None # No piece is selected
                        self.pieceKey = None # Then there is no piece key
                        self.reColorSq()
    ...
    def reColorSq(self):
        for square in self.recordColor: 
            square.setColor(self.recordColor[square])
...
#             print(square.getz())
            ...
            self.recordColor[square] = roundTuple(square.getColor())
        ...
        for piece in self.pCord:
            ...
            self.inverseP[self.pCord[piece]] = piece
#         print(self.squares)
            ...
            for x in range(-3,5): # For changing col
                for y in range(13,5,-1):
                    ...
                    if count % 2 == 0: # Even count will be black squares
                        ...
                        self.inverseP[(cord)] = None
                        self.inverseP[(cord[0], cord[1], cord[2] * -1)] = None
